 ...
 * dependencies and thus can only be run under OS-X.
 *
 ...
 *
 * Note: This class depends on https://github.com/CocoaPods/CocoaPods/pull/10609 which is not yet release, so a build
...
class CocoaPods(
    ...
    private val podspecCache: MutableMap<String, Podspec> = mutableMapOf()
        ...
        if (lockfile.isFile) {
            ...
            issues += createAndLogIssue(
                ...
                message = "Missing lockfile '${lockfile.relativeTo(analysisRoot).invariantSeparatorsPath}' for " +
                        "definition file '${definitionFile.relativeTo(analysisRoot).invariantSeparatorsPath}'.",
            )
        }
    ...
    private fun getPodspec(id: Identifier, workingDir: File): Podspec {
        ...
        val podspecFile = run("spec", "which", podspecName, "--version=${id.version}", "--allow-root",
